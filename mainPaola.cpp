...
// Guilty Gear
Model ApartamentoAbandonado_M;
Model Axl_M;
Model BaikenWorkshop_M;
Model BancaSteampunk_M;
Model BoteSteampunk_M;
Model EdificioAbandonado_M;
Model LucesSteampunk_M;
Model Millia_M;
Model PortBreeze_M;
Model RelojSteampunk_M;
Model Sol_M;
Model Tirnanog_M;
Model TrenAzul_M;
...
int main()
{
	...
	// Guilty Gear
	ApartamentoAbandonado_M = Model();
	ApartamentoAbandonado_M.LoadModel("Models/GuiltyGear/apartamento_abandonado.glb");

	Axl_M = Model();
	Axl_M.LoadModel("Models/GuiltyGear/axl_guilty.glb");

	BaikenWorkshop_M = Model();
	BaikenWorkshop_M.LoadModel("Models/GuiltyGear/baiken_wokshop.glb");

	BancaSteampunk_M = Model();
	BancaSteampunk_M.LoadModel("Models/GuiltyGear/banca_steampunk.glb");

	BoteSteampunk_M = Model();
	BoteSteampunk_M.LoadModel("Models/GuiltyGear/bote_steampunk.glb");

	EdificioAbandonado_M = Model();
	EdificioAbandonado_M.LoadModel("Models/GuiltyGear/edificio_abandonado.glb");

	LucesSteampunk_M = Model();
	LucesSteampunk_M.LoadModel("Models/GuiltyGear/luces_steampunk.glb");

	Millia_M = Model();
	Millia_M.LoadModel("Models/GuiltyGear/millia_guilty.glb");

	PortBreeze_M = Model();
	PortBreeze_M.LoadModel("Models/GuiltyGear/port_breeze.glb");

	RelojSteampunk_M = Model();
	RelojSteampunk_M.LoadModel("Models/GuiltyGear/reloj_steampunk.glb");

	Sol_M = Model();
	Sol_M.LoadModel("Models/GuiltyGear/sol_guilty.glb");

	Tirnanog_M = Model();
	Tirnanog_M.LoadModel("Models/GuiltyGear/tirnanog.glb");

	TrenAzul_M = Model();
	TrenAzul_M.LoadModel("Models/GuiltyGear/tren_azul.glb");
...
// --- SOL BADGUY ---
		...
		// --- AXL ---
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(85.0f, -1.0f, 40.0f));
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1, 0, 0));
		model = glm::scale(model, glm::vec3(4.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glDisable(GL_CULL_FACE);
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glDisable(GL_CULL_FACE);
		ApartamentoAbandonado_M.RenderModel();
		glEnable(GL_CULL_FACE);

		// --- EDIFICIO ABANDONADO ---
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(220.0f, -1.0f, -180.0f));
		model = glm::rotate(model, glm::radians(-40.0f), glm::vec3(0, 1, 0));
		model = glm::scale(model, glm::vec3(12.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glDisable(GL_CULL_FACE);
		EdificioAbandonado_M.RenderModel();
		glEnable(GL_CULL_FACE);
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glDisable(GL_CULL_FACE);
		RelojSteampunk_M.RenderModel();
		glEnable(GL_CULL_FACE);

		// --- BANCA STEAMPUNK ---
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-15.0f, -1.0f, -85.0f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0, 1, 0));
		model = glm::scale(model, glm::vec3(3.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glDisable(GL_CULL_FACE);
		BancaSteampunk_M.RenderModel();
		glEnable(GL_CULL_FACE);

		// --- TREN AZUL ---
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-250.0f, -1.0f, -50.0f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0, 1, 0));
		model = glm::scale(model, glm::vec3(8.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glDisable(GL_CULL_FACE);
		TrenAzul_M.RenderModel();
		glEnable(GL_CULL_FACE);
