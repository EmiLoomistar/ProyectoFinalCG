...
uniform int useTexture;  // 0 = color plano (sin textura), 1 = muestrear textura
uniform int usaBaked;    // 1 = textura cocinada, no aplicar iluminación dinámica
...
vec4 CalcPointLight(PointLight pLight)
{
	...
	float attenuation = pLight.exponent * distance * distance +
						...
						pLight.constant;
	if (attenuation < 0.0001) attenuation = 0.0001;
...
void main()
{
	vec4 texColor = (useTexture == 0) ? vec4(1.0, 1.0, 1.0, 1.0) : texture(theTexture, TexCoord);

	if (noLighting == 1) {
		color = vec4(texColor.rgb, texColor.a * vColor.a);
	} else if (usaBaked == 1) {
		// Textura cocinada: la iluminación ya está integrada, no multiplicar por luz dinámica
